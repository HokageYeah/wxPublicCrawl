# AI模块快速使用指南

## 🎯 问题解决

如果遇到 `ModuleNotFoundError: No module named 'app'` 错误，已经修复！

## ✅ 正确的使用方法

### 1️⃣ 配置环境变量

在项目根目录创建 `.env.desktop` 文件（如果还没有）：

```bash
# 在项目根目录执行
cd /Users/yuye/YeahWork/Python项目/wxPublicCrawl

# 创建配置文件
cat > .env.desktop << EOF
AI_API_KEY=your-openai-api-key
AI_BASE_URL=https://api.openai.com/v1
AI_MODEL=gpt-3.5-turbo
EOF
```

### 2️⃣ 运行快速测试（推荐第一次运行）

```bash
# 确保在项目根目录
cd /Users/yuye/YeahWork/Python项目/wxPublicCrawl

# 激活虚拟环境（如果还没激活）
source venv/bin/activate

# 运行快速测试
python app/ai/test/quick_test.py
```

这会测试：
- ✓ AI配置是否正确
- ✓ AI客户端是否可用
- ✓ JSON响应功能
- ✓ 提示词管理器

### 3️⃣ 运行完整示例

```bash
# 在项目根目录运行
python app/ai/test/usage_examples.py
```

这会运行8个完整示例：
1. 基础AI对话
2. JSON格式响应
3. 流式响应
4. 多轮对话（带历史）
5. 提示词管理
6. PromptBuilder使用
7. 内容分类（实际业务）
8. 错误处理

## 📝 在你的代码中使用

### 示例1: 简单对话

```python
from app.ai.llm.ai_client import AIClient

async def my_function():
    client = AIClient()
    response = await client.chat(user_message="你好")
    print(response)
```

### 示例2: 使用教育分析功能

```python
from app.ai.code.education_analyze import analyze_education_articles
from app.schemas.wx_data import ArticleSimple

articles = [
    ArticleSimple(aid="1", title="小学数学教学"),
    ArticleSimple(aid="2", title="今日新闻")
]

education_ids = await analyze_education_articles(articles)
# 返回: ["1"]
```

## 🔧 常见问题

### Q1: 提示 API Key 未配置

**解决方法：**
1. 检查 `.env.desktop` 文件是否存在
2. 确保文件中有 `AI_API_KEY=xxx`
3. 重新运行程序

### Q2: 提示连接超时

**可能原因：**
1. 网络问题
2. API Base URL 配置错误

**解决方法：**
```bash
# 如果使用代理，配置 AI_BASE_URL
# 如果在国内，可能需要使用国内的 OpenAI 代理服务
AI_BASE_URL=https://your-proxy-url/v1
```

### Q3: JSON 解析失败

**原因：** AI 返回的内容不是有效的 JSON 格式

**解决方法：**
1. 降低 temperature（如 0.1）提高稳定性
2. 在提示词中明确要求只返回 JSON
3. 使用 `chat_with_json_response()` 方法，它会自动清理 Markdown 代码块

## 📚 进一步学习

- 详细文档: `app/ai/README.md`
- 重构说明: `app/ai/REFACTOR_SUMMARY.md`
- 代码示例: `app/ai/test/usage_examples.py`

## 🎓 添加自己的AI功能

### 步骤1: 创建提示词文件

```bash
# 创建提示词
cat > app/ai/prompt/my_feature_prompt.txt << EOF
你是一个专业的分析助手。
请分析以下数据：
{{ data }}

返回JSON格式: {"result": "..."}
EOF
```

### 步骤2: 创建分析函数

```python
# app/ai/code/my_feature_analyze.py
from app.ai.llm.ai_client import AIClient
from app.ai.utils.prompt_manager import get_prompt_manager

async def analyze_my_feature(data: str):
    # 加载提示词
    manager = get_prompt_manager('app/ai/prompt')
    prompt = manager.render_prompt("my_feature_prompt", data=data)
    
    # 调用AI
    client = AIClient(temperature=0.3)
    result = await client.chat_with_json_response(
        user_message=prompt,
        system_message="你是一个专业的分析助手"
    )
    
    return result
```

### 步骤3: 在API中使用

```python
from app.ai.code.my_feature_analyze import analyze_my_feature

@router.post("/my-feature-analyze")
async def api_analyze(data: str):
    result = await analyze_my_feature(data)
    return {"code": 0, "data": result}
```

## 💡 提示

1. **测试前先运行快速测试**：`python app/ai/test/quick_test.py`
2. **查看日志**：程序会自动记录详细的日志信息
3. **调整温度参数**：
   - 分类任务用 0.1-0.3（更稳定）
   - 对话任务用 0.7-0.9（更自然）
   - 创作任务用 0.9-1.2（更有创意）

---

**更新时间**: 2025-12-26  
**状态**: ✅ 已修复导入错误

