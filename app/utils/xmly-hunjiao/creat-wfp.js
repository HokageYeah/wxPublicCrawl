...
class XimalayaWFPGenerator {
    ...
    /**
     * 加载核心混淆代码
     * 注意：必须先加载 dws.js (包含 a0_0x1957)，再加载 hx.js
     */
    loadCoreCode() {
        try {
            const fs = require('fs');
            ...
            // 2. 再加载 hx.js (依赖 a0_0x1957)
            const hxPath = path.join(dir, 'hx.js');
            if (fs.existsSync(hxPath)) {
                const hxCode = fs.readFileSync(hxPath, 'utf8');
                eval(hxCode);
                console.log('✓ hx.js 加载成功');
            } else {
                throw new Error('hx.js 文件不存在');
            }
    ...
    generate() {
        ...
        try {
            ...
            let $ats = typeof window.$ats !== 'undefined' ? window.$ats : 
                      (typeof global.$ats !== 'undefined' ? global.$ats : null);
            
            if (!$ats) {
                throw new Error('$ats 对象未找到，请确保 hx.js 和 dws.js 都已正确加载');
            }
...
if (require.main === module) {
    ...
    try {
        ...
        wfpList.forEach((wfp, index) => {
            ...
            console.log(`  [${index + 1}] ${wfp.substring(0, 32)}... ${isUnique ? '✓' : '✗ (重复)'}`);
        });
        console.log('');
        ...
        console.log(`  重复率: ${((wfpList.length - uniqueSet.size) / wfpList.length * 100).toFixed(2)}%`);
        console.log('');
        ...
        console.error('  2. 确保 hx.js 文件存在于当前目录');
