...
```mermaid
graph TB
    User[👤 用户] --> |交互| Window[🪟 桌面窗口<br/>PyWebView]
    Window --> |加载| Frontend[🎨 Vue3 前端<br/>web/dist]
    Frontend --> |HTTP| Backend[⚙️ FastAPI 服务<br/>127.0.0.1:18000]
    Backend --> |业务逻辑| Service[📦 wx_public.py]
    Service --> |网络请求| WxServer[☁️ 微信服务器<br/>mp.weixin.qq.com]
    Service --> |数据持久化| SQLite[💾 SQLite<br/>本地数据库]
    Service --> |文件操作| FileSystem[📁 本地文件<br/>用户目录]
...
sequenceDiagram
    ...
    participant Lock as 🔒 单实例检查
    participant Server as ⚙️ FastAPI 服务
    participant Window as 🪟 PyWebView 窗口
    
    User->>App: 双击启动
    App->>Lock: 检查实例与端口
    Lock-->>App: ✓ 无冲突
    App->>Server: 启动后台服务<br/>(127.0.0.1:18000)
    Server-->>App: ✓ 服务就绪
    App->>Window: 创建窗口并加载前端
    Window-->>User: 显示登录界面
```
...
1. **环境检查** (5秒内完成)
   ...
   - 检查单实例锁文件（`~/Library/Application Support/wx公众号工具/app.lock`）
   - 检查端口 18000 可用性
   - 初始化 SQLite 数据库
...
def get_writable_dir(subdir='temp'):
    ...
    if platform.system() == 'Darwin':  # macOS
        base = '~/Library/Application Support/wx公众号工具'
    elif platform.system() == 'Windows':
        base = '~/AppData/Local/wx公众号工具'
    else:  # Linux
        base = '~/.local/share/wx公众号工具'
