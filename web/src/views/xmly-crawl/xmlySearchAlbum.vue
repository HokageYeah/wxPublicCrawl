  ...
  >
    <div class="max-w-4xl mx-auto px-6 py-12">
      ...
      <header class="mb-12 text-center relative">
        <div
          class="absolute inset-0 -z-10 bg-gradient-to-r from-orange-500/10 to-red-500/10 blur-3xl opacity-30 rounded-full transform scale-150"
        ></div>
        <h1
          class="text-4xl font-bold mb-4 text-white tracking-tight flex items-center justify-center gap-4"
        >
          <span
            class="p-3 bg-white/5 rounded-2xl border border-white/10 shadow-2xl backdrop-blur-sm"
          >
            <span
              class="i-carbon-microphone-filled text-orange-500 text-3xl block"
            ></span>
          </span>
          喜马拉雅专辑订阅
        </h1>
      ...
      <!-- Main Control Card -->
      <div
        class="bg-[#111] border border-gray-800 rounded-2xl p-1 shadow-2xl overflow-hidden mb-8 group hover:border-gray-700 transition-colors duration-300"
      >
        <div class="bg-[#151515] rounded-xl p-8 relative overflow-hidden">
          <!-- Glass effect decoration -->
          <div
            class="absolute top-0 right-0 p-32 bg-orange-500/5 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 pointer-events-none"
          ></div>
          ...
          <div class="grid md:grid-cols-2 gap-8 items-center relative z-10">
            ...
            <div>
              ...
              <div class="flex items-center gap-4 group/id">
                <span
                  class="text-4xl font-mono font-bold text-white tracking-tight group-hover/id:text-orange-400 transition-colors"
                >
                  {{ testAlbumId }}
                </span>
                <span
                  v-if="loading"
                  class="flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 text-xs text-orange-400"
                >
            ...
            <div class="flex flex-col sm:flex-row gap-4 justify-end">
              ...
              >
                <span
                  class="i-carbon-trash-can group-hover/del:scale-110 transition-transform"
                ></span>
                取消订阅
              </button>
            </div>
      ...
      <!-- Terminal Log -->
      <div
        class="bg-[#0f0f0f] rounded-xl border border-gray-800 overflow-hidden shadow-2xl font-mono text-sm relative group"
      >
        <!-- Terminal Header -->
        <div
          class="bg-[#1a1a1a] px-4 py-3 flex items-center justify-between border-b border-gray-800"
        >
        ...
        <!-- Terminal Body -->
        <div
          class="p-4 h-80 overflow-y-auto bg-black/50 custom-scrollbar tracking-wide"
        >
          <transition-group name="terminal">
            ...
            >
              <span
                class="opacity-30 select-none w-20 text-xs mt-0.5 shrink-0 text-right font-light"
                >{{ log.time }}</span
              >
              <span class="opacity-50 select-none mt-0.5">❯</span>
              <span
                class="group-hover/log:opacity-100 opacity-90 transition-opacity"
                >{{ log.msg }}</span
              >
            </div>
          </transition-group>
          ...
          <div
            v-if="logs.length === 0"
            class="h-full flex flex-col items-center justify-center text-gray-800"
          >
            <div class="i-carbon-terminal text-4xl mb-2 opacity-20"></div>
            <p class="text-xs uppercase tracking-widest opacity-30">
