<template>
  <div class="h-screen w-full bg-[#0a0a0a] text-gray-200 font-sans flex items-center justify-center overflow-hidden py-8">
    <!-- 固定宽高的主容器 -->
    <div class="max-w-6xl w-full h-full max-h-[calc(100vh-4rem)] flex flex-col bg-[#0a0a0a]">
      
      <!-- 搜索区域卡片 (参考 SearchPublic.vue 样式) -->
      <div class="card p-6 bg-[#141414] border border-gray-800 shadow-lg flex-none">
        <div class="flex items-center gap-4 mb-4">
          <!-- Brand / Title -->
          <div class="flex items-center gap-2 select-none">
             <div class="w-10 h-10 rounded-lg bg-gradient-to-br from-orange-500 to-red-600 flex items-center justify-center shadow-lg shadow-orange-900/20">
               <span class="i-carbon-microphone-filled text-white text-xl"></span>
             </div>
      ...
      <div class="flex-1 overflow-hidden mt-6 relative">
        <div class="absolute inset-0 overflow-y-auto scroll-smooth custom-scrollbar px-4 py-2">
          
          <!-- Loading State -->
          <div v-if="loading && !results.length" class="h-full flex flex-col items-center justify-center min-h-[400px]">
          ...
          <!-- Empty State: Initial -->
          <div v-else-if="!hasSearched" class="h-full flex flex-col items-center justify-center min-h-[400px] text-center opacity-40">
             <span class="i-carbon-music text-7xl text-gray-600 mb-6"></span>
             <h3 class="text-xl font-bold text-gray-300 mb-2">探索有声世界</h3>
             <p class="text-gray-500 max-w-sm">输入关键词，发现海量优质有声书与精选专辑</p>
          </div>

          <!-- Empty State: No Results -->
          <div v-else-if="results.length === 0" class="h-full flex flex-col items-center justify-center min-h-[400px] text-center opacity-60">
             <span class="i-carbon-search-locate text-7xl text-gray-600 mb-6"></span>
               ...
               >
                 ...
                 <div class="aspect-square relative overflow-hidden bg-gray-900">
                   ...
                   <div class="absolute top-2 right-2 flex flex-col gap-1.5 items-end z-10">
                     <span v-if="album.isPaid" class="px-2 py-0.5 bg-yellow-600/90 backdrop-blur-sm text-white text-[10px] font-bold rounded shadow-sm">付费</span>
                     <span v-if="album.isFinished === 2" class="px-2 py-0.5 bg-green-600/90 backdrop-blur-sm text-white text-[10px] font-bold rounded shadow-sm">完结</span>
                     <span v-if="album.isVipFree" class="px-2 py-0.5 bg-red-600/90 backdrop-blur-sm text-white text-[10px] font-bold rounded shadow-sm">VIP</span>
                   </div>

                   <!-- Overlay Info -->
                   <div class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black via-black/60 to-transparent p-3 pt-10">
                      <div class="flex items-center text-xs text-gray-300 gap-3 font-mono">
                         <span class="flex items-center gap-1"><span class="i-carbon-play-filled text-orange-500"></span>{{ formatNumber(album.playCount) }}</span>
                         <span class="flex items-center gap-1"><span class="i-carbon-playlist text-orange-500"></span>{{ album.tracksCount }}集</span>
                      </div>
                   </div>
                 </div>
                 ...
                 <div class="p-4 flex flex-col flex-1">
                   ...
                   <p class="text-xs text-gray-400 mb-4 line-clamp-2 leading-relaxed min-h-[2.5em]">
                     {{ album.intro || '暂无简介' }}
                   </p>
                   
                   <!-- Anchor -->
                   <div class="flex items-center gap-2 mb-4 mt-auto border-t border-white/5 pt-3">
                      <img :src="album.anchorPic" class="w-6 h-6 rounded-full object-cover bg-gray-800" />
                      <span class="text-xs text-gray-400 truncate flex-1 hover:text-gray-200">{{ album.nickname }}</span>
                   </div>
                   ...
                   >
                     ...
                     <template v-else>
                       <span :class="isSubscribed(album.albumId) ? 'i-carbon-checkmark-filled' : 'i-carbon-add-alt'"></span>
                       <span>{{ isSubscribed(album.albumId) ? '已订阅' : '订阅监控' }}</span>
                     </template>
                   </button>
                 </div>
               </div>
          ...
          <!-- 分页区域 -->
          <div v-if="hasSearched && pagination" class="mt-8 pt-6 border-t border-gray-800">
             <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div class="text-sm text-gray-500">
                   共找到 <span class="text-orange-500 font-bold mx-1">{{ pagination.total }}</span> 条结果
                </div>
                
                <!-- Pagination Controls -->
                <div v-if="pagination.totalPage > 1" class="flex items-center gap-2">
                   <button
                   ...
                   <div class="px-4 py-1.5 bg-[#1a1a1a] rounded-md border border-gray-800 text-sm flex items-center gap-1 text-gray-400 font-mono">
                      <span class="text-orange-500 font-bold">{{ pagination.currentPage }}</span>
    ...
    >
      <div 
        v-if="toast" 
        class="fixed top-24 right-6 z-[100] max-w-sm w-full bg-[#1a1a1a]/90 backdrop-blur-md border shadow-2xl rounded-lg pointer-events-auto ring-1 ring-black ring-opacity-5 overflow-hidden"
        :class="{
          ...
          <div class="flex items-start">
            <div class="flex-shrink-0">
              <span 
                class="text-xl"
                :class="{
                  'i-carbon-checkmark-filled text-green-400': toast.type === 'success',
                  'i-carbon-warning-filled text-red-400': toast.type === 'error',
                  'i-carbon-information-filled text-blue-400': toast.type === 'info'
                }"
              ></span>
            </div>
            ...
            <div class="ml-4 flex-shrink-0 flex">
              <button 
                @click="toast = null"
                class="rounded-md inline-flex text-gray-400 hover:text-gray-200 focus:outline-none"
              >
...
import type { SearchAlbumResult, SearchAlbumResponse, SearchAlbumPagination } from '@/types/xmly';
...
const highlightKeyword = (text: string) => {
  ...
  try {
     ...
     return text.replace(reg, match => `<span class="text-orange-500 font-medium bg-orange-500/10 rounded px-0.5">${match}</span>`);
  } catch (e) {
     return text;
  }
};
...
const performSearch = async (page: number) => {
  ...
  try {
    ...
    if (typeof res === 'string') {
        try { data = JSON.parse(res); } catch { throw new Error('API Response Parse Error'); }
    } else {
        data = res;
    }
...
const toggleSubscribe = async (album: SearchAlbumResult) => {
  ...
  try {
    if (isSubscribed(id)) {
      ...
      if (res && (res.ret === 200 || JSON.stringify(res).includes('成功') || res.code === 0)) {
