<template>
  <div class="min-h-screen bg-black text-white">
    ...
    <!-- 页面标题 -->
      ...
      <!-- 激活配置卡片 -->
      <div v-if="activeConfig" class="mt-6 p-4 bg-blue-900/20 border border-blue-500/30 rounded-lg">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-3">
            <span class="i-carbon-circle-check text-2xl text-green-400"></span>
            <div>
              <div class="text-sm text-gray-400">当前激活配置</div>
              <div class="font-semibold text-lg">{{ activeConfig.model_name }}</div>
            </div>
          </div>
          <div class="flex items-center gap-4">
            <span class="px-3 py-1 text-xs font-medium bg-green-500/20 text-green-400 rounded-full">
              {{ activeConfig.model_type }}
            </span>
            <span class="text-sm text-gray-400">{{ activeConfig.description || '无描述' }}</span>
          </div>
        </div>
      </div>
      <div v-else class="mt-6 p-4 bg-yellow-900/20 border border-yellow-500/30 rounded-lg">
        <div class="flex items-center gap-3">
          <span class="i-carbon-warning text-2xl text-yellow-400"></span>
          <div>
            <div class="text-sm text-gray-400">暂无激活配置</div>
            <div class="font-medium text-yellow-400">请创建并激活一个配置以使用AI功能</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="loading" class="flex items-center justify-center py-20">
      <span class="i-carbon-circle-dash animate-spin text-5xl text-blue-500"></span>
    </div>

    <div v-else-if="configs.length === 0" class="flex flex-col items-center justify-center py-20 border-2 border-dashed border-gray-800 rounded-lg">
      ...
      >
        ...
        <div class="relative p-5 bg-gray-800 border-b border-gray-700">
          <div class="flex items-start justify-between">
            <div class="flex items-center gap-3">
              ...
              <div>
                <h3 class="font-bold text-lg text-white group-hover:text-blue-400 transition-colors">{{ config.model_name }}</h3>
                <span class="text-xs text-gray-400">{{ config.model_type }}</span>
              </div>
            </div>
            <div v-if="config.is_active" class="px-2 py-1 bg-green-500/20 border border-green-500/30 rounded-full flex items-center gap-1">
              <span class="i-carbon-circle-check text-xs text-green-400"></span>
        ...
        <div class="p-5 space-y-3">
          <div class="flex items-center gap-2 text-sm">
            ...
            <span class="text-gray-500 font-mono">{{ maskedApiKey(config.ai_api_key) }}</span>
          </div>
          <div class="flex items-center gap-2 text-sm">
            <span class="i-carbon-link text-gray-400"></span>
            <span class="text-gray-300 font-medium">Base URL:</span>
            <span class="text-gray-500 truncate">{{ config.ai_base_url || '未设置' }}</span>
          </div>
          <div class="flex items-center gap-2 text-sm">
        ...
        <!-- 卡片操作 -->
        <div class="px-5 py-4 bg-gray-800/50 border-t border-gray-700 flex items-center justify-between">
          <button
    ...
    <!-- 分页 -->
    <div v-if="total > filters.limit" class="mt-8 flex items-center justify-center gap-4">
      <button
        @click="changePage(-1)"
    ...
    >
      <div class="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden shadow-2xl">
        <!-- 对话框头部 -->
        <div class="px-6 py-4 border-b border-gray-700 flex items-center justify-between bg-gray-800">
          ...
          <form @submit.prevent="saveConfig" class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="col-span-2">
                <label class="block text-sm font-medium text-gray-300 mb-1">配置描述 <span class="text-red-400">*</span></label>
                <input
              ...
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-1">模型类型 <span class="text-red-400">*</span></label>
                <select
              ...
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-1">模型名称 <span class="text-red-400">*</span></label>
                <input
            ...
            <!-- API 配置 -->
            <div class="space-y-4">
              <h3 class="text-base font-semibold text-gray-300 flex items-center gap-2">
                <span class="i-carbon-api text-blue-400"></span>
                API 配置
              </h3>

              <div>
                <label class="block text-sm font-medium text-gray-300 mb-1">API Key <span class="text-red-400">*</span></label>
                <div class="relative">
                  <input
            ...
            <div class="space-y-4">
              ...
              <div class="grid grid-cols-2 gap-4">
                <div>
                  <label class="block text-sm font-medium text-gray-300 mb-1">Temperature ({{ formData.temperature }})</label>
                  <input
                ...
                <div>
                  <label class="block text-sm font-medium text-gray-300 mb-1">Top P ({{ formData.top_p }})</label>
                  <input
              ...
              <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label class="block text-sm font-medium text-gray-300 mb-1">Max History</label>
                  <input
            ...
            <div class="space-y-4">
              ...
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-1">系统提示词</label>
                <textarea
              ...
              <div>
                <label class="block text-sm font-medium text-gray-300 mb-1">自定义参数 (JSON)</label>
                <textarea
        ...
        <!-- 对话框底部 -->
        <div class="px-6 py-4 border-t border-gray-700 flex items-center justify-end gap-3 bg-gray-800/50">
          <button
            @click="closeDialog"
    ...
    <Transition name="toast">
      <div
        v-if="toast.show"
        :class="[
          'fixed bottom-8 right-8 px-6 py-4 rounded-lg shadow-2xl flex items-center gap-3 z-50',
          toast.type === 'success' ? 'bg-green-600 text-white' : 'bg-red-600 text-white'
        ]"
      >
        <span :class="toast.type === 'success' ? 'i-carbon-checkmark-filled' : 'i-carbon-error-filled'"></span>
...
const fetchModelTypes = async () => {
  try {
    const result = await request.post<{ model_types: string[] }>('/llm-config/llm-config-model-types');
    modelTypes.value = result.model_types || [];
  } catch (error: any) {
    console.error('获取模型类型失败:', error);
    // 检查是否是表不存在的错误
    const errorMessage = error.message || '';
    if (errorMessage.includes('no such table') || errorMessage.includes('llm_configuration')) {
      // 表不存在，静默处理
...
const fetchConfigs = async () => {
  loading.value = true;
  try {
    const result = await request.post<{ total: number; items: LLMConfig[] }>('/llm-config/llm-config-list', {
      user_id: null,
...
const fetchActiveConfig = async () => {
  try {
    const result = await request.post<LLMConfig>('/llm-config/llm-config-active', { user_id: null });
...
const openEditDialog = async (config: LLMConfig) => {
  try {
    // 获取完整配置信息
    const result = await request.post<LLMConfig>('/llm-config-get', {
      config_id: config.id,
      user_id: null
    });
...
const saveConfig = async () => {
  ...
  try {
    // 验证必填字段
    if (!formData.value.description || !formData.value.model_type || !formData.value.model_name || !formData.value.ai_api_key) {
...
const activateConfig = async (configId: number) => {
  ...
  try {
    ...
    const errorMessage = error.response?.data?.detail || error.message || '激活失败';
    showToast(errorMessage, 'error');
  } finally {
    loading.value = false;
  }
